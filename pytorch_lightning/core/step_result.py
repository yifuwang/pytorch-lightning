...
class _ResultItem(dict):
    # TODO: remove data?'
    data: Any
    meta: _ResultMetadata = field(repr=False)
...
class Result(dict):
    ...
    def _parse_key(key: str) -> str:
        key = Result.removesuffix(key, '_epoch')
        key = Result.removesuffix(key, '_step')
        return key

    def __getitem__(self, key: str) -> _ResultItem:
    ...
    def extract_batch_size(batch: Any) -> int:
        try:
            return Result.unpack_batch_size(batch)
    ...
    @staticmethod
    def unpack_batch_size(batch: Any) -> int:
        """
